...
function getRandomDateWithinLast2Hours() {
    const now = new Date();
    const millisecondsIn2Hours = 2 * 60 * 60 * 1000;
    const randomMilliseconds = Math.floor(Math.random() * millisecondsIn2Hours);
		...
		for(var h in this.headers) {
			...
			if(hl == 'subject' && v === null) {
				...
				if (obfuscateArticles && hl == 'date') {
					v = getRandomDateWithinLast2Hours();
				}
			}
